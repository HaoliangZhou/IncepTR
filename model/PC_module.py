...
class HeadDropOut(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training and self.p > 0 and self.upper > 0:
            ...
            batch_rand = np.random.rand(B)
            batch_rand = torch.from_numpy(batch_rand)
...
class Attention_raw(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = (attn.matmul(v)).transpose((0, 2, 1, 3)) #改天试试这种写法
        # x = x.reshape((B, N, C))
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
# 带有head drop的attention
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., head_drop_type='Dropout',
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'head_drop'):
            ...
            # qkv_masd = self.head_drop(qkv)
        # qkv = qkv + qkv_masd  # 两个attention相加(残差),发现效果其实不好
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 head_drop_type='Dropout', head_drop_rate=0.0, head_drop_upper=1):
        super().__init__()
...
class VisionTransformer_POS(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., head_drop_type='HeadDropOut', head_drop_rate=0.0, head_drop_upper=1,
                 drop_path_rate=0.15, hybrid_backbone=None, norm_layer=None):
        """
        Args:
