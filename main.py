...
# from model.DualATTNet import DualATTNet_IncepCBAM as DualATTNet
from model.DualATTNet import DualATTNet_ViT as DualATTNet
...
def arg_process():
    ...
    parser.add_argument('--gpuid', default='cuda:1', help='the gpu index for training')
...
def train_model(model, dataloaders, criterion, optimizer, device='cpu', num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for j, samples in enumerate(dataloaders):
            ...
            loss = criterion(output_class, class_labels)  # 计算output_class的loss
...
def main():
    ...
    logPath = os.path.join('result', model_name + '_3ls_log_cde2019_vit-msad.txt')
    ...
    rstPath = os.path.join('result', model_name + '_lr' + str(lr) + '_rst_vit-msad.mat')
